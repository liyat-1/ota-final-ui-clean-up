  ...
  return (
    <section className="mt-8 border-t border-zinc-200 pt-6">
      ...
      <div className="mt-3 rounded-xl border border-blue-100 bg-blue-50/60 p-4">
        <span className="inline-flex items-center gap-1.5 rounded-md bg-emerald-100 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-emerald-700">
          <Sparkles size={11} /> {offerHeadlineValue(offer)}
        </span>
        <p className="mt-2.5 text-[14px] font-semibold text-zinc-900">{offer.title}</p>
        <p className="mt-1.5 text-[12.5px] leading-relaxed text-zinc-600">{offer.description}</p>
        <p className="mt-2.5 text-[11.5px] text-zinc-500">{offer.validity}</p>
      </div>
    </section>
  );
}
